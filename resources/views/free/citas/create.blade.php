...
<div class="container mx-auto p-6 max-w-lg">
    <h1 class="text-3xl font-bold mb-6">Agregar Nueva Cita</h1>

    <form action="{{ route('free.citas.store') }}" method="POST" class="space-y-4">
        @csrf

        <div>
            <label for="fecha" class="block font-semibold mb-1">Fecha:</label>
            <input type="date" name="fecha" id="fecha" value="{{ old('fecha') }}" required class="w-full border rounded px-3 py-2" />
            @error('fecha')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="hora" class="block font-semibold mb-1">Hora:</label>
            <input type="time" name="hora" id="hora" value="{{ old('hora') }}" class="w-full border rounded px-3 py-2" />
            @error('hora')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="motivo" class="block font-semibold mb-1">Motivo:</label>
            <input type="text" name="motivo" id="motivo" value="{{ old('motivo') }}" required class="w-full border rounded px-3 py-2" />
