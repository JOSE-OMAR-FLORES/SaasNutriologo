  ...
  <div class="container mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row justify-between items-center">
      <div class="mb-4 md:mb-0">
        <h1 class="text-2xl md:text-3xl font-bold text-emerald-800 animate-bounce">Panel Free : Mejora tu Subcripcion ya!</h1>
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md border-l-4 border-emerald-400 hover:shadow-lg transition-all">
      <div class="flex items-center">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-emerald-600">{{ $pacientes->total() }}</p>
        </div>
      </div>
    </div>
  ...
  <form method="GET" action="{{ route('free.dashboard') }}" class="mb-6">
  <input 
    type="text" 
    name="filter_nombre" 
  ...
  />
  <button type="submit" class="mt-2 px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700 transition">
    Filtrar
  </button>
</form>
...
<nav class="inline-flex -space-x-px rounded-md shadow-sm text-emerald-600" aria-label="Pagination">
  {{ $pacientes->appends(request()->except('pacientes_page'))->links() }}
</nav>
  ...
  <section class="mb-10">
          ...
          @foreach($citas as $cita)
            <div class="p-4 hover:bg-blue-50 transition">
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-2">
                  <a href="{{ route('free.citas.show', $cita) }}" class="text-emerald-600 hover:text-emerald-700 p-1 transform hover:scale-110 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
    ...
    <nav class="inline-flex -space-x-px rounded-md shadow-sm text-emerald-600" aria-label="Pagination">
  {{ $citas->appends(request()->except('pacientes_page'))->links() }}
</nav>
  </section>
  ...
  <section>
              ...
              <div class="flex justify-between items-start">
                ...
                <div class="flex items-center space-x-2">
                  <a href="{{ route('free.notas.show', $nota) }}" class="text-purple-600 hover:text-purple-700 p-1 transform hover:scale-110 transition">
                    ...
                    <a href="{{ route('free.notas.edit', $nota) }}" class="text-blue-600 hover:text-blue-700 p-1 transform hover:scale-110 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  </a>
    ...
    <nav class="inline-flex -space-x-px rounded-md shadow-sm text-emerald-600" aria-label="Pagination">
  {{ $notas->appends(request()->except('pacientes_page'))->links() }}
</nav>
  </section>
</div>
